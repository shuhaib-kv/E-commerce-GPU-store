...
// func Razorpaynow(c *gin.Context) {
...
// 	}
// }
// func RazorpaynowSuccess(c *gin.Context) {
...
func RazorPay(c *gin.Context) {
	...
	database.Db.Where("users_id = ?", user.ID).Find(&orders)

	client := razorpay.NewClient("rzp_test_Nfnipdccvgb8fW", "UfwKXCGjiUrcfTEXpWlupcrN")
	razpayvalue := orders.TotalAmount * 100
	data := map[string]interface{}{
		"amount":   razpayvalue,
		"currency": "INR",
		"receipt":  "some_receipt_id",
	...
	c.HTML(200, "app.html", gin.H{
		...
		"UserID":       user.ID,
		"total_price":  orders.TotalAmount,
		"total":        razpayvalue,
		"orderid":      value,
